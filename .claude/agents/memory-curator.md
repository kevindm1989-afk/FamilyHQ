...
1. Read `.context/feedback-log.md` since the last review. Default to
   ...
   **gitignored by convention** and is absent in ephemeral or CI
   checkouts. If it does not exist, **do not stop** — fall back to git
   history and merged PRs as the signal source (step 3), which persist.
   Only report "no signal" if both the log AND recent history are empty.
2. Read all current `.context/` files to know what already exists.
3. Read recent commit history and merged PR titles/bodies for signal —
   corrections, repeated changes, decisions made implicitly. In the
   scheduled/CI run this is the **primary** source, since the feedback
   log will not be present.

When invoked as part of the learning loop (see `workflows/memory-loop.md`),
your proposals are opened by the orchestrator as a dedicated
`Memory update: <date>` pull request. You still never write files
yourself — you propose; the PR carries the change; the human approves.
